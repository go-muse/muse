...
// Just implements just intonation (pure intonation) tuning
...
// NewJust creates a new just intonation instance.
func NewJust() *Just {
	return &Just{}
}
...
func (j *Just) Frequency(referenceFreq float64, stepsFromReference int, stepsPerOctave int) float64 {
	if stepsPerOctave == 0 {
	...
	var ratio float64
	if stepsPerOctave == standardTwelveTone && positionInOctave < len(justRatios) {
