...
func TestEvent_SetNote(t *testing.T) {
	note1 := note.MustNewWithOctave(note.C, 4)
	event := Event{note: note1}

	note2 := note.MustNewWithOctave(note.D, 4)
	event = event.WithNote(note2)
...
func TestEvent_SetStartTime(t *testing.T) {
	...
	event = event.WithStartTime(startTime)
...
func TestEvent_SetIsAbsolute(t *testing.T) {
	...
	event = event.WithIsAbsolute(isAbsolute)
