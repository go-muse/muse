...
func TestTrack_Play(t *testing.T) {
	...
	track.events = []*Event{
		{
			startTime:  1,
			note:       note.C.NewNote().WithDuration(1),
			isAbsolute: true,
		},
	}
	...
	testCases := []testCase{
		{
			events: []*Event{
				{startTime: 1, note: note.C.NewNote().WithDuration(3), isAbsolute: true},
			},
			want: []*PlayEvent{
				{
					Event:     &Event{startTime: 1, note: note.C.NewNote().WithDuration(3), isAbsolute: true},
					eventType: EventTypeStart,
					time:      1,
				},
				{
		...
		},
		{
			events: []*Event{
				{startTime: 1, note: note.C.NewNote().WithDuration(3), isAbsolute: true},
				{startTime: 2, note: note.D.NewNote().WithDuration(1), isAbsolute: true},
			},
			want: []*PlayEvent{
				{
					Event:     &Event{startTime: 1, note: note.C.NewNote().WithDuration(3), isAbsolute: true},
				...
				{
					Event:     &Event{startTime: 2, note: note.D.NewNote().WithDuration(1), isAbsolute: true},
					eventType: EventTypeStart,
					time:      2,
				},
				{
