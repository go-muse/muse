...
func ExampleTrack_AddNote() {
	...
	noteC := note.MustNewWithOctave(note.C, octave.Number4).WithDuration(time.Second)
	track.AddNote(
		noteC,
		0,
		true,
	...
	noteD := note.MustNewWithOctave(note.C, octave.Number4).WithValue(duration.NewRelative(duration.NameHalf))
	track.AddNote(
		noteD,
		noteC.Duration(),
		false,
